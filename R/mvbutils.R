...
#    copy.ns.objects( cq( fix.list, maintained.packages), pkgname)
  ...
  # Generated by 'make_CRANtidote'
  # Remove for (better) non-CRAN versions on R-universe
  # Antidote for silliterate CRAN warnings about no-visible-globals
  # if( T/F) debugging!
  if( TRUE) sapply( c(
      '%dopar%', '.', '..', '.First.lib', '.Last.lib', '.Saved.plots', '.SavedPlots',
      '.Traceback', 'author', 'b', 'B', 'backup.fix', 'binary', 'blah', 'bod',
      'bug.position', 'Built', 'calc_xpos', 'ChangeLog', 'CHANGES', 'character.only', 'chtml',
      'cleanup', 'Cloaders', 'compress.lazyload', 'configure', 'CONTENTS', 'DAY',
      'dir.', 'DLL', 'dll.paths', 'dlls', 'doc', 'docType', 'dont.check.visibility',
      'dynamic.help', 'env', 'ewhere', 'EXAMPLES', 'exclude.data', 'exclude.funs', 'exec',
      'execute.First', 'execute.Last', 'export.me', 'exports.have.changed', 'fixing',
      'force.all.docs', 'funmat', 'funs', 'generics', 'getDoParName', 'highlight',
      'html', 'i', 'importing_Clink', 'INDEX', 'inst', 'INSTALL', 'installed', 'ipath',
      'is.Rd2', 'last.R.major', 'last.warning', 'latex', 'level', 'libs', 'LICENCE',
      'LICENSE', 'lldir', 'load.from', 'm', 'man', 'MD5', 'meta', 'mfilename',
      'mfiles', 'mods.in.packages', 'mods.in.tasks', 'MONTH', 'MP', 'mvb.session.info',
      'mvbutils', 'my.reps', 'n', 'name', 'name.load.from', 'NAMESPACE', 'NEWS',
      'orig.path', 'outdir', 'override.answer', 'package', 'pkg', 'prepend', 'prune',
      'Rd.version', 'Rdir', 'ROOT', 'Rrebver', 'run_postcopy_hooks', 's', 'sep', 'sourcedir',
      'spath', 'src', 'stopCluster', 'tarball', 'task', 'task.path', 'task.tree',
      'tasks', 'tests', 'textcolor', 'to', 'trace.was.on', 'tricky_dicky', 'type.load.from',
      'USAGE', 'useDynLib', 'Version', 'vignettes', 'vscode', 'w', 'what', 'where',
      'x', 'y', 'Y', 'YEAR', 'YES'
    ), assign, value= function(...) stop( "CRANtidote"), envir=environment( sys.function()))
...
stop( "No longer allowed to 'cd' into maintained packages-- if you must, then first use 'unmaintain.package'")
  ...
  # 2026 edit to fix fixes...
  tasks <- mget( 'tasks', pos=2, ifnotfound= list( character(0)))[[1]]
  
  # Old version, which worked fine for 25 years: would pick up implicitly from top search()
  # since new empty env has just been added
  # if( !exists("tasks", where = 2, inherits = FALSE)) tasks <- character(0)
...
stop( "Can't find an image file to load for '" %&% taskname %&% "'!")
  ...
  # save.refdb OK because can't m-trace in m.p. itself
...
stopifnot( length( o1)>0, length( o2)>0) # not in both
    CRANbeater <- get( 're' %&% 'quire' %&% 'Namespace', baseenv())
    if( !CRANbeater( 'diffr')){
stop( "Couldn't load package 'diffr'")
    ...
    for( a in atts( o1)){
      cat( c( '', sprintf( 'attribute(%s)=', a), attr( o1, a)), sep='\n', tf1, append=TRUE)
    }
    writeLines( o2[[ c( 2,2)]], tf2)
    for( a in atts( o2)){
      cat( c( '', sprintf( 'attribute(%s)=', a), attr( o2, a)), sep='\n', tf2, append=TRUE)
    }
...
return( NULL)
  ...
  # Next line is COMPLETELY POINTLESS except to defeat stupid CRAN no-global NOTE
  makeR3 <- asNamespace( 'RcppTidy')$makeR3
...
return( structure( character( 0), for.info='No modifications (but some updated files)'))
  ...
  is.traced <- is_mtraced( answer)
  if( any( is.traced)){
    cat( 'Reapplying trace(s)...')
    lapply( answer[ is.traced], debug::mtrace, fname=NULL, # fname=NULL forces char.fname
        from=.GlobalEnv, # NOT ideal, but until "from" is added to trace info, ...
        # ... better than the alternative, which goes straight to baseenv
        tracing=TRUE)
    cat( 'done\n')
  }
...
function() {
  # 2026 edit to fix fixes...
  tasks <- mget( 'tasks', pos=1, ifnotfound= list( structure( character(0), names=character(0))))[[1]]

  # Old version, perfectly good for 25 years (and always, unless baseenv acquires something called "tasks"...)
...
# Used to just have re_bloody_quire() here, but I've had to add fake definitions of 
# detectCores etc via make_CRANtidote to subvert spurious NOTE. Therefore, either need to use 
# double-colons (as I have), or locally_import(), but the latter is hardly worth it for 2 calls!

stopifnot( re_bloody_quire( 'parallel')) # or crash
...
"is_mtraced" <-
function( things){
  if( 'package:debug' %in% search()){
    tracees <- asNamespace( 'debug')$tracees
    mtraced <- match( things, tracees, 0) > 0
  } else {
    mtraced <- rep( FALSE, length( things))
  }
return( mtraced)
}
...
As of 2023, I've forgotten what that's about, and/or why mlocal() wraps its evaluand in a fake repeat-loop...
  ...
  # NB in R4.1, next does not find the right frame when debugging (with m-trace)
...
"locally_import" <-
function( pkg, funs=getNamespaceExports( pkg), to=parent.frame()){
...
return( FALSE)
  ...
  for( iex in funs){
    assign( iex, getExportedValue( ns, iex), envir=to)
  }
return( TRUE)
}
...
structure( function( ..., character.only=FALSE, autopatch=FALSE){
  ...
  if( length( packs)) {
    ...
    # Next doesn't need inherits=F, since parent is EmptyEnv
...
"make_CRANtidote" <-
structure( function( checkout){
  if( !is.character( checkout <- attr( checkout, 'output'))){
stop( "'checkout' should be result of 'check.pkg(<mypak>)' w/w-o 'CRAN=T' arg")
  }
  
  stupid <- 'no visible (binding for global variable|global function definition for)'
  noglob1 <- grepv( stupid %&% " '", checkout) |> sub( ".* '", "'", x=_)
      
  inoglob2 <- grep( stupid %&% '$', checkout)
  noglob2 <- if( length( inoglob2)) 
      checkout[ inoglob2+1] |> sub( "^ *", "", x=_) |> sub( " +$", "", x=_) 
    else
      character()
  noglob <- unique( c( noglob1, noglob2))
  len_stupid <- length( noglob)

  # Deffo *don't* try to define these, even if they cause a problem...
  # Just one for now...
  exceptions <- sprintf( "'%s'", c( '.onAttach')) %that.are.in% noglob
  if( length( exceptions)){
    scatn( '"Unblockables" found--- fix these manually: %s', paste( exceptions, collapse=', '))
  }
  noglob <- sort( noglob %except% exceptions)
  if( !length( noglob)){
return( as.cat( '  # Nothing to see here'))
  }
  
  noglob <- paste( noglob, collapse= ', ')
  
  if( len_stupid > 1){
    # Split long lines roughly every 80 chars...
    gap1 <- gregexpr( "', '", noglob[[1]], fixed=TRUE)[[1]]
    mm <- findInterval( seq( from=80, by=80, to=max( 80, tail( gap1, 1))), gap1)
    migmog <- substring( noglob, c( 1, gap1[ mm]+3), 
        c( gap1[ mm]+1, nchar( noglob)) )
  } else { # just one
    migmog <- noglob
  }
  
  noglob <- c( '  # Antidote for S-illiterate CRAN warnings about no-visible-globals',
      '  sapply( c(', '      ' %&% migmog, 
      '    ), assign, value= function(...) stop( "CRANtidote"), envir=environment( sys.function()))')
as.cat( noglob)
}
, doc =  docattr( r"{
make_CRANtidote    package:mvbutils
locally_import 
...
'locally_import' might be generally useful for packages in "Suggests", even without 'make_CRANtidote' (if you don't care about CRAN), since it saves on precious colons--- and it _does_ eliminate the unlikely-but-logically-possible problem that a synonymous function from some completely different attached package _might_ mask the version you are hoping to call (and without 'locally_import', the colonless version would only work if you 'attach(otherpak)' anyway). You can also "import" just some selected functions.
...
ARGUMENTS
 ...
 checkout: Result of a call to 'check.pkg'; AFAIK, it doesn't matter here whether the 'CRAN' argument is set.

 pkg: name of a package, presumably listed in "Suggests" but not necessarily
 
 funs: which things to "import" from 'pkg'. Default is everything that's exported by 'pkg'. Usually it's only functions that might be imported, but I think it's conceivable that non-functions might be exported--- which is fine, despite the name 'funs'.
 
 to: environment to create the "imports" in. The default is whatever frame 'locally_import' is called from--- so the copy will almost always be temporary. In the somewhat unlikely event that you are calling 'locally_import' inside 'lapply' or similar, you might need to specify e.g. 'to=environment()'.
...
structure( function( Clink_list, dir) {
  tsub <- function() stop( "This is broken; sorry. Some kind of template substitution was intended")
...
#print( expr)
  ...
  # As of R4.1, Should only be needed by m-trace
...
function(
  ...
  if( PARALLEL){
    # Temp copies of exportees; colon-free refs without side-effects from make_CRANtidote()
    PARALLEL <- locally_import( 'foreach') & locally_import( 'parallel') & 
      locally_import( 'iterators') & locally_import( 'doParallel')
  }
...
# call any others. No logical reason for this, but may improve clarity.
  ...
  #   Space out function names ppnl to # of letters
      ...
      # 2026 note: the next line looks funny, and was leading to probs with make_CRANtidote. 
      # Presumably, to stop really short names getting hidden.
      # I wonder if it was an ancient hack-provision?
      # So, commented out for 2026, since I can't see any way for the user to set it!
      # if( exists( 'minstrl', frame=sys.nframe())) nch <- pmax( nch, minstrl)
...
#    mvb.file.copy( file.path( dir., makes.in.top), file.path( sourcedir, makes.in.top))
  ...
  default.list <- c( copies, returnList(
      ...
      exclude.data=  c(
          ...
          # Avoiding cq() next cozza STUPID CRAN note sensitivity, and 
          # these objects might be slightly delicate for make_CRANtidote()
          "forced!exports", ".required", ".Depends", "tasks", ".Traceback", 
              ".packageName", "last.warning", ".Random.seed", ".SavedPlots", ".Last.value" 
        ),
      task.path=pkg,
  ...
  if( !has.namespace) {
    ...
    undoc.funs <- funs %except% c( find.documented( ewhere, doctype='any'),
        cq( .First.lib, .Last.lib, .onLoad, '.onAttach')) # re quotes, see 'make_CRANtidote'--- sigh
...
return(invisible(NULL))
  ...
  # 2026: looks like this could do with a streamlining
  if( ('package:debug' %in% search()) && exists( 'tracees', 'package:debug')
      && length( pos.tracees <- debug::check.for.tracees( pos))) {
    retracees <- pos.tracees[ is_mtraced( pos.tracees)] # 2026; was %that.are.in% names( tracees)
    restoro <- sapply( named( retracees), get, envir=pos)
    temp.unmtraced <- asNamespace( 'debug')$tracees[ retracees]
    on.exit( {
    ...
    retrace.envs <- lapply( named( retracees), debug::mtrace, fname=NULL, tracing=FALSE, 
    ...
    lapply( pos.tracees %except% retracees, debug::mtrace, fname=NULL, tracing=FALSE, 
...
return( list( handle=handle, trigger=emptyenv()))
  ...
  # Next line of code is COMPLETELY UNNECESSARY to functionality
  finalizer <- handle <- 'AVOID_STUPID_CRAN_FALSE_POSITIVE_NOTE' # speaks for itself
...
stop( "Not overwriting")
  ...
  ## Update tasks (2026 version)...
  tasks <- mget( 'tasks', envir=.GlobalEnv, ifnotfound= list( character(0)))[[1]]
  # old, fine,  was: if( is.null( tasks <- .GlobalEnv$tasks)) tasks <- character( 0)
